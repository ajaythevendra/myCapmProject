module.exports = cds.service.impl(async function () {
    ...
    //Syntax for generic handler
    // this.WHEN('WhichOperation', What_Entity,Code)
    this.before(['CREATE', 'UPDATE'], EmployeeSet, async (req, res) => {
        console.log("aa gaya salary" + req.data.salaryAmount);
        if (parseFloat(req.data.salaryAmount) >= 1000000) {
            req.error(500, "Chacha Bahut jyada salary dala h!");
        }
    })

    this.after(['CREATE', 'UPDATE'], EmployeeSet, async (req, res) => {
        console.log("deke aa gaya salary" + res.data.salaryAmount);
